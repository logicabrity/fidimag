...
cdef extern from "dipolar.h":
    ...
    ctypedef struct fft_demag_plan:
        pass
    ctypedef struct fft_demag_plan_scalar:
        pass
    ...
    fft_demag_plan_scalar * create_plan_scalar()
...
cdef class FFTDemagScalar(object):
    cdef fft_demag_plan_scalar *_c_plan


    #tensor_type could be 'dipolar', 'demag' or '2d_pbc'
    def __cinit__(self, dx, dy, dz, nx, ny, nz):
        self._c_plan = create_plan_scalar()
        if self._c_plan is NULL:
            raise MemoryError()
        init_plan_scalar(self._c_plan, dx, dy, dz, nx, ny, nz)
        compute_dipolar_tensors_scalar(self._c_plan)
        create_fftw_plan_scalar(self._c_plan)
        compute_demag_tensors_scalar(self._c_plan)
    ...
    def __dealloc__(self):
        if self._c_plan is not NULL:
            finalize_plan_scalar(self._c_plan)
            self._c_plan = NULL

    def compute_field(self,np.ndarray[double, ndim=1, mode="c"] spin,
                        np.ndarray[double, ndim=1, mode="c"] mu_s,
                        np.ndarray[double, ndim=1, mode="c"] field):
        compute_fields_scalar(self._c_plan, &spin[0], &mu_s[0], &field[0])
    ...
    def compute_energy(self,
        ...
        return compute_demag_energy_scalar(self._c_plan, &spin[0], &mu_s[0], &field[0])